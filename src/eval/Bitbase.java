...
public class Bitbase {
	...
	private int index(boolean white2Move, int wKingIndex, int bKingIndex,
			int pIndex) {
		return wKingIndex + (bKingIndex << 6) + ((white2Move ? 1 : 0) << 12)
				+ ((7 - (pIndex % 8)) << 13) + ((6 - (pIndex >> 3)) << 15);
	}

	// Pre-computed values are analysed to determine the best move
	public boolean probe(int wKingIndex, int bKingIndex, int wPawnIndex,
			boolean white2Move) {
		int index = index(white2Move, Board.mirrorIndex(wKingIndex),
				Board.mirrorIndex(bKingIndex), Board.mirrorIndex(wPawnIndex));
		return ((EvalConstants.BITBASE[index / 32]
				& (1 << (index & 0x1F)))) != 0;
	}
