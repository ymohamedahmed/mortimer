...
public class EvalInfo {
	...
	private void checkPinnerRay(long ray, long mines,
			long attackSlider) {
	...
	private void checkPinnerBishop(int kingIndex, long bishopAttacks,
			long all, long mines, long bishopsOrQueens, int side) {
		if ((bishopAttacks & mines) == 0
				|| (CoreConstants.BISHOP_TABLE[kingIndex]
						& bishopsOrQueens) == 0) {
			return;
		}
		long xray = getBishopMoves(board,
				all & ~(mines & bishopAttacks), kingIndex, side);
	...
	private void checkPinnerRook(int kingIndex, long rookAttacks,
			long all, long mines, long rooksOrQueens, int side) {
		if ((rookAttacks & mines) == 0
				|| (CoreConstants.ROOK_TABLE[kingIndex]
						& rooksOrQueens) == 0) {
			return;
		}
		long xray = getRookMoves(board, all & ~(mines & rookAttacks),
				kingIndex, side);
		if ((xray & ~rookAttacks & rooksOrQueens) != 0) {
			int rank = kingIndex / 8;
			int file = kingIndex % 8;
			checkPinnerRay(xray & CoreConstants.ROW_UPWARD[rank],
					mines, rooksOrQueens);
			checkPinnerRay(xray & CoreConstants.LEFT_FILES[file],
					mines, rooksOrQueens);
			checkPinnerRay(xray & CoreConstants.ROW_DOWNARD[rank],
					mines, rooksOrQueens);
			checkPinnerRay(xray & CoreConstants.RIGHT_FILES[file],
					mines, rooksOrQueens);
		}
	}
	...
	public void generate(BitBoard board) {
		this.board = board;
		kingIndex[0] = BitBoard.bitScanForward(
				board.bitboards[CoreConstants.WHITE_KING]);
		kingIndex[1] = BitBoard.bitScanForward(
		...
		bishopAttackKing[0] = getBishopMoves(board, all, kingIndex[0],
				0);
		checkPinnerBishop(kingIndex[0], bishopAttackKing[0], all,
				whites, (bishops | queens) & blacks, 0);
		bishopAttackKing[1] = getBishopMoves(board, all, kingIndex[1],
				1);
		checkPinnerBishop(kingIndex[1], bishopAttackKing[1], all,
				blacks, (bishops | queens) & whites, 1);
		...
		for (int index = 0; index < 64; index++) {
			if ((square & all) != 0) {
				long moves = 0;
				int color = (board.bitboards[CoreConstants.WHITE]
						& square) != 0 ? CoreConstants.WHITE
								: CoreConstants.BLACK;
	...
	private long getBishopMoves(BitBoard board, long all, int index,
			int side) {
		if (index != -1) {
	...
	private long getRookMoves(BitBoard board, long all, int index,
			int side) {
		if (index != -1) {
			long rookBlockers = all
