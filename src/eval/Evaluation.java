...
public class Evaluation extends EvalConstants {
	...
	public int evaluate(BitBoard board, int color) {
		try {
			...
			int whitePawns = BitBoard
					.hammingWeight(board.bitboards[CoreConstants.WHITE_PAWN]);
			int blackPawns = BitBoard
					.hammingWeight(board.bitboards[CoreConstants.BLACK_PAWN]);
			int whiteKnights = BitBoard
					.hammingWeight(board.bitboards[CoreConstants.WHITE_KNIGHT]);
			int blackKnights = BitBoard
					.hammingWeight(board.bitboards[CoreConstants.BLACK_KNIGHT]);
			int whiteBishops = BitBoard
					.hammingWeight(board.bitboards[CoreConstants.WHITE_BISHOP]);
			int blackBishops = BitBoard
					.hammingWeight(board.bitboards[CoreConstants.BLACK_BISHOP]);
			int whiteRooks = BitBoard
					.hammingWeight(board.bitboards[CoreConstants.WHITE_ROOK]);
			int blackRooks = BitBoard
					.hammingWeight(board.bitboards[CoreConstants.BLACK_ROOK]);
			int whiteQueens = BitBoard
					.hammingWeight(board.bitboards[CoreConstants.WHITE_QUEEN]);
			int blackQueens = BitBoard
					.hammingWeight(board.bitboards[CoreConstants.BLACK_QUEEN]);
			// Work out the endgame value, if the game has not reached the
			// endgame the function will return no value
			int endgameValue = Endgame.evaluate(board, scaleFactor, whitePawns,
					blackPawns, whiteKnights, blackKnights, whiteBishops,
					blackBishops, whiteRooks, blackRooks, whiteQueens,
					blackQueens);
			...
			int gamePhase = nonPawnMaterial >= MAT_MIDGAME_MAX ? PHASE_MIDGAME
					: (nonPawnMaterial <= MAT_ENDGAME_MIN) ? PHASE_ENDGAME
							: ((nonPawnMaterial - MAT_ENDGAME_MIN)
									* PHASE_MIDGAME)
			...
			if (gamePhase > 0) {
				...
				long whiteBehindPawn = ((whitePawnsBoard >>> 8)
						| (whitePawnsBoard >>> 16) | (whitePawnsBoard >>> 24));
				long blackBehindPawn = ((blackPawnsBoard << 8)
						| (blackPawnsBoard << 16) | (blackPawnsBoard << 24));
			...
			for (int i = 0; i < 5; i++) {
				...
				pawnCanAttack[0] |= ((whitePawnsBoard
						& ~CoreConstants.FILE_A) << 9)
						| ((whitePawnsBoard & ~CoreConstants.FILE_H) << 7);
				pawnCanAttack[1] |= ((blackPawnsBoard
						& ~CoreConstants.FILE_H) >>> 9)
			...
			attacks[1] = evalAttacks(board, ei, 1,
			...
			for (int index = 0; index < 64; index++) {
				...
				if ((square & all) != 0) {
					boolean isWhite = ((board.bitboards[CoreConstants.WHITE]
							& square) != 0);
					int col = isWhite ? 0 : 1;
					...
					if ((square & pawns) != 0) {
						...
						long otherPawnsAheadAdjacent = ranksForward
								& adjacentFiles & otherPawns;
						...
						boolean passed = !doubled && !opposed
						...
						if (!passed) {
							long myPawnsAheadAdjacent = ranksForward
							...
							boolean backward = !isolated && !candidate
									...
									&& (CoreConstants.ROW_BACKWARD_INCLUSIVE[col][isWhite
											? (int) BitBoard.bitScanForward(
													myPawnsAheadAdjacent) / 8
											: (int) BitBoard.bitScanBackward(
													myPawnsAheadAdjacent) / 8]
											& routeToPromotion & (pawns
													| ei.pawnAttacks[enemy])) != 0;
							if (backward) {
								pawnStruct[col] -= PAWN_BACKWARDS[opposed ? 1
										: 0];
							}
							if (isolated) {
								pawnStruct[col] -= PAWN_ISOLATED[opposed ? 1
										: 0];
							}
							if (doubled) {
								pawnStruct[col] -= PAWN_DOUBLED[opposed ? 1
										: 0];
							}
							...
							if ((square & (CoreConstants.FILE_D
							...
							if (gamePhase > 0 && relativeRank > 2) {
								long stormPawns = otherPawnsAheadAdjacent
										& ~CoreConstants.FILE_D
										& ~CoreConstants.FILE_E;
								if (stormPawns != 0) {
									int otherKingFile = ei.kingIndex[enemy] % 8;
									if ((stormPawns
											& CoreConstants.FILE[otherKingFile]) != 0) {
							...
							long attackedNotDefendedRoute = ((routeToPromotion
									...
									| ((backFile & (rooks | queens)
											& others) != 0 ? routeToPromotion
													: 0))
									& ~((routeToPromotion
											& ei.attackedSquares[col])
											| ((backFile & (rooks | queens)
													& mines) != 0
															? routeToPromotion
															: 0));
							boolean connected = ((CoreConstants.KING_TABLE[index]
									& board.bitboards[enemy]) & adjacentFiles
									& myPawns) != 0;
							boolean outside = otherPawns != 0 && (((square
									& CoreConstants.LEFT_FILES[3]) != 0
									&& (pawns
											& CoreConstants.LEFT_FILES[file]) == 0)
									|| ((square
											& CoreConstants.RIGHT_FILES[4]) != 0
											&& (pawns
													& CoreConstants.RIGHT_FILES[file]) == 0));
							boolean mobile = (pushSquare
									& (all | attackedNotDefendedRoute)) == 0;
							...
							if (relativeRank >= 2) {
								...
								passedPawns[col] += Board.distance(pushIndex,
										ei.kingIndex[enemy])
										* PAWN_PASSER_OTHER_KING_DISTANCE[relativeRank]
										- Board.distance(pushIndex,
												ei.kingIndex[col])
						...
						if (gamePhase > 0 && (pawnFile & ~ranksForward
								& kingZone[col] & ~CoreConstants.FILE_D
								& ~CoreConstants.FILE_E) != 0) {
						...
						mobility[col] += MOBILITY[KNIGHT][BitBoard
								.hammingWeight(
										safeAttacks & mobilitySquares[col])];
						...
						if ((square & OUTPOST_MASK[col]
								& ~pawnCanAttack[enemy]) != 0) {
							positional[col] += KNIGHT_OUTPOST[(square
									* ei.pawnAttacks[col]) != 0 ? 1 : 0];
						}
						// Consider bishops
						...
						if ((square & OUTPOST_MASK[col]
								& ~pawnCanAttack[enemy]) != 0) {
							positional[col] += BISHOP_OUTPOST[(square
									* ei.pawnAttacks[col] != 0 ? 1 : 0)];
						}
						// Penalise if the bishop is blocked by a pawn
						positional[col] -= BISHOP_MY_PAWNS_IN_COLOR_PENALTY
								* BitBoard.hammingWeight(pawns & mines
										& ((square & WHITE_SQUARES) != 0
												? WHITE_SQUARES
												: BLACK_SQUARES));
						...
						if ((square * OUTPOST_MASK[col]
								& ~pawnCanAttack[enemy]) != 0) {
							positional[col] += ROOK_OUTPOST[(square
									* ei.pawnAttacks[col]) != 0 ? 1 : 0];
						}
						long rookFile = CoreConstants.FILE[file];
						if ((rookFile & pawns & mines) == 0) {
							positional[col] += ROOK_FILE[(rookFile & pawns) == 0
									? 0 : 1];
						}
						if (relativeRank >= 4) {
							long alignedPawns = CoreConstants.ROW[rank] & pawns
									& others;
							if (alignedPawns != 0) {
								positional[col] += ROOK_7
										* BitBoard.hammingWeight(alignedPawns);
							}
						}
						...
						mobility[col] += MOBILITY[QUEEN][BitBoard.hammingWeight(
								safeAttacks & mobilitySquares[col])];
						kingAttacks = safeAttacks & kingZone[enemy];
			...
			int openingAndEnding = (white2Move ? TEMPO : -TEMPO) + pawnMat[0]
					- pawnMat[1] + nonPawnMat[0] - nonPawnMat[1]
					+ pieceSquare[0] - pieceSquare[1] + spatial[0] - spatial[1]
					+ positional[0] - positional[1] + attacks[0] - attacks[1]
					+ mobility[0] - mobility[1] + pawnStruct[0] - pawnStruct[1]
					+ passedPawns[0] - passedPawns[1]
					+ openingEndingWithShift(6,
							KING_SAFETY_PONDER[kingAttackedCount[0]]
									* kingSafety[0]
									- KING_SAFETY_PONDER[kingAttackedCount[1]]
											* kingSafety[1]);
			// Consider the phase of the game
			int value = (gamePhase * open(openingAndEnding)
					+ (PHASE_MIDGAME - gamePhase) * end(openingAndEnding)
	...
	int evalAttacks(BitBoard board, EvalInfo ei, int color, long enemy) {
		...
		long otherWeak = ei.attackedSquares[color] & enemy
				& ~ei.pawnAttacks[1 - color];
		if (otherWeak != 0) {
			// Consider minor(knight and bishop) and major(rook and queen)
			// pieces that have attacks available
			long attackedByMinor = (ei.knightAttacks[color]
					| ei.bishopAttacks[color]) & otherWeak;
			...
			long attackedByMajor = (ei.rookAttacks[color]
					| ei.queenAttacks[color]) & otherWeak;
		...
		long superiorAttacks = ei.pawnAttacks[color] & enemy & ~pawns
				| (ei.knightAttacks[color] | ei.bishopAttacks[color]) & enemy
						& (rooks | queens)
	...
	public int openingEndingWithShift(int shiftValue, int openEndingValue) {
		return S(open(openEndingValue) >> shiftValue,
				end(openEndingValue) >> shiftValue);
	}
