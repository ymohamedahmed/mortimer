...
public class Evaluation extends EvalConstants {
	...
	// Counts for the number of pieces
	int whitePawns;
	int blackPawns;
	int whiteBishops;
	int blackBishops;
	...
	int whiteRooks;
	int blackRooks;
	int whiteNonPawnValue;
	int blackNonPawnValue;

	public int evaluate(BitBoard board, int color) {
		try {
			int materialScore = material(board);
			int endgameValue = endgame(board);
			if (endgameValue != NO_VALUE) {
				return endgameValue;
			}
			int nonPawnMaterial = end(whiteNonPawnValue + blackNonPawnValue);
			...
			mobility[0] = 0;
			mobility[1] = 0;
			kingAttackedCount[0] = 0;
			kingAttackedCount[1] = 0;
			kingSafety[0] = 0;
			kingSafety[1] = 0;
			pawnStruct[0] = 0;
			pawnStruct[1] = 0;
			...
			if (gamePhase > 0) {
				...
				long blackBehindPawn = ((blackPawnsBoard << 8) | (blackPawnsBoard << 16) | (blackPawnsBoard << 24));
				spatial[0] = SPACE
						* (((BitBoard.hammingWeight(whiteSafe) + BitBoard.hammingWeight(whiteSafe & whiteBehindPawn))
								* (whiteKnights + whiteBishops)) / 4);
				spatial[1] = SPACE
						* (((BitBoard.hammingWeight(blackSafe) + BitBoard.hammingWeight(blackSafe & blackBehindPawn))
				...
				if ((square & all) != 0) {
					...
					if ((square & pawns) != 0) {
						...
						if (!passed) {
							...
							boolean backward = !isolated && !candidate && myPawnsBesideAndBehindAdjacent == 0
									...
									&& (CoreConstants.ROW_BACKWARD_INCLUSIVE[col][isWhite
											...
											: (int) BitBoard.bitScanBackward(myPawnsAheadAdjacent) / 8]
											& routeToPromotion & (pawns | ei.pawnAttacks[enemy])) != 0;
							if (backward) {
							...
							boolean connected = ((CoreConstants.KING_TABLE[index] & board.bitboards[enemy])
									& adjacentFiles & myPawns) != 0;
						...
						if ((BISHOP_TRAPPING[index] & pawns & others) != 0) {
							mobility[col] -= BISHOP_TRAPPED_PENALTY[(BISHOP_TRAPPING_GUARD[index] & pawns & others) != 0
									? 1 : 0];
						}
			...
			int openingAndEnding = (white2Move ? TEMPO : -TEMPO) + materialScore + pieceSquare[0] - pieceSquare[1]
					+ spatial[0] - spatial[1] + positional[0] - positional[1] + attacks[0] - attacks[1] + mobility[0]
					- mobility[1] + pawnStruct[0] - pawnStruct[1] + passedPawns[0] - passedPawns[1]
					+ openingEndingWithShift(6, KING_SAFETY_PONDER[kingAttackedCount[0]] * kingSafety[0]
	...
	private int material(BitBoard board) {
		whitePawns = BitBoard.hammingWeight(board.bitboards[CoreConstants.WHITE_PAWN]);
		blackPawns = BitBoard.hammingWeight(board.bitboards[CoreConstants.BLACK_PAWN]);
		whiteKnights = BitBoard.hammingWeight(board.bitboards[CoreConstants.WHITE_KNIGHT]);
		blackKnights = BitBoard.hammingWeight(board.bitboards[CoreConstants.BLACK_KNIGHT]);
		whiteBishops = BitBoard.hammingWeight(board.bitboards[CoreConstants.WHITE_BISHOP]);
		blackBishops = BitBoard.hammingWeight(board.bitboards[CoreConstants.BLACK_BISHOP]);
		whiteRooks = BitBoard.hammingWeight(board.bitboards[CoreConstants.WHITE_ROOK]);
		blackRooks = BitBoard.hammingWeight(board.bitboards[CoreConstants.BLACK_ROOK]);
		whiteQueens = BitBoard.hammingWeight(board.bitboards[CoreConstants.WHITE_QUEEN]);
		blackQueens = BitBoard.hammingWeight(board.bitboards[CoreConstants.BLACK_QUEEN]);
		int whitePawnValue = (whitePawns * PIECE_VALUE_PHASE[PAWN]);
		int blackPawnValue = (blackPawns * PIECE_VALUE_PHASE[PAWN]);
		whiteNonPawnValue = (whiteBishops * PIECE_VALUE_PHASE[BISHOP]) + (whiteRooks * PIECE_VALUE_PHASE[ROOK])
				+ (whiteQueens * PIECE_VALUE_PHASE[QUEEN]) + ((whiteBishops == 2) ? BISHOP_PAIR : 0);
		blackNonPawnValue = (blackKnights * PIECE_VALUE_PHASE[KNIGHT]) + (blackBishops * PIECE_VALUE_PHASE[BISHOP])
				+ (blackRooks * PIECE_VALUE_PHASE[ROOK]) + (blackQueens * PIECE_VALUE_PHASE[QUEEN])
				+ ((blackBishops == 2) ? BISHOP_PAIR : 0);
		int whiteMat = whitePawnValue + whiteNonPawnValue;
		int blackMat = blackPawnValue + blackNonPawnValue;
		return whiteMat - blackMat;
	}

	private int endgame(BitBoard board) {
