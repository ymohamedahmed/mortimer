...
public class Endgame extends EvalConstants {
	...
	public static int evaluate(BitBoard board, int[] scaleFactor, int wPawns, int bPawns,
			int wKnights, int bKnights, int wBishops, int bBishops, int wRooks, int bRooks,
		...
		if (bMat == 0 && (wBishops >= 2 || wRooks > 0 || wQueens > 0)
			...
			// or a queen or rook
			return Endgame.endgameKingWithNoPieces(board, wMat > bMat, wKnights + bKnights,
					wBishops + bBishops, wRooks + bRooks, wQueens + bQueens);
		}
		if (wRooks == 1 && bRooks == 1
				&& ((wPawns == 2 && bPawns == 1) || (wPawns == 1 && bPawns == 2))) {
	...
	private static int endgameKingWithNoPieces(BitBoard board, boolean wDominating, int knights,
			int bishops, int rooks, int queens) {
	...
	private static int endgameKingPawnKing(BitBoard board, boolean wDominating) {
		...
		return wDominating
				? KNOWN_WIN + PAWN
						+ (int) BitBoard.bitScanForward(board.bitboards[CoreConstants.WHITE_PAWN])
								/ 8
				: -KNOWN_WIN - PIECE_VALUE[PAWN] - (7
						- (int) BitBoard.bitScanForward(board.bitboards[CoreConstants.BLACK_PAWN])
								/ 8);
	}
	...
	private static int scaleKingRookPawnKingRook(BitBoard board, boolean wDominating) {
		...
		if ((CoreConstants.ROW[rank7] & pawns) != 0
				...
				&& (CoreConstants.ROW_BACKWARD_INCLUSIVE[domCol][rank2] & otherRook) != 0
				&& (white2Move != wDominating) || (Board.distance(pIndex, domkingIndex) >= 2)) {
			return SCALE_FACTOR_DRAW;
		}
	...
	private static int endgameKingQueenKingPawn(BitBoard board, boolean wDominating) {
		...
		long pawnZone = 0;
		// Based on the column of the pawn calculate the pawn zone
	...
	private static int endgameKingBishopPawnKingBishop(BitBoard board, boolean wDominating) {
		...
		long otherBishopSquares = ((otherBishop & WHITE_SQUARES) != 0) ? WHITE_SQUARES
				: BLACK_SQUARES;
