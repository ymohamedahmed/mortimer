...
public class Endgame extends EvalConstants {
		...
		if (wPawns == 0 && bPawns == 0) {
			...
			if (wNonPawnMat == 1 && bNonPawnMat == 1) {
				if (wRooks == 1 && bRooks == 1) {
					...
					// accordingly
					scaleFactor[0] = scaleKingRookPawnKingRook(board,
							wPawns > bPawns);
				}
				if (wBishops == 1 && bBishops == 1) {
					// One side has a bishop and pawn and the other one bishop
					return endgameKingBishopPawnKingBishop(board,
							wPawns > bPawns);
				}
	...
	private static int endgameKingPawnKing(BitBoard board,
			boolean wDominating) {
	...
	private static int scaleKingRookPawnKingRook(BitBoard board,
		...
		if ((CoreConstants.ROW[rank7] & pawns) != 0
				&& (CoreConstants.ROW_FORWARD[domCol][rank6] & pFile
						& otherKing) != 0
				&& (CoreConstants.ROW_BACKWARD_INCLUSIVE[domCol][rank2]
						& otherRook) != 0
	...
	private static int endgameKingQueenKingPawn(BitBoard board,
		...
		if ((pawnZone & enemyKing) != 0
				&& Board.distance(domKingIndex, pIndex) >= 1) {
			return DRAW;
		}
	...
	private static int endgameKingBishopPawnKingKnight(BitBoard board,
			boolean wDominating) {
		int domCol = wDominating ? 0 : 1;
		int enemyCol = domCol == 0 ? 1 : 0;
	...
	private static int endgameKingBishopPawnKingBishop(BitBoard board,
			boolean wDominating) {
		int domCol = wDominating ? 0 : 1;
		int enemyCol = domCol == 0 ? 1 : 0;
		long domBishop = board.bitboards[domCol
