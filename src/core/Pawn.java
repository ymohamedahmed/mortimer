...
public class Pawn extends Piece {
    ...
    public boolean isPawnPromotion(ArrayList<Piece> pieceList) {
        ArrayList<Move> possibleMoves = getMovesList();
        for (Move move : possibleMoves) {
            if ((move.getPosition().getRow() == 7 && getColor() == PieceColor.WHITE) || (move.getPosition().getRow() == 0 && getColor() == PieceColor.BLACK)) {
                return true;
            }
        }
        return false;
