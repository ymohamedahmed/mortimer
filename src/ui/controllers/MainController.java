...
public class MainController {
				...
				.addListener(
						new ChangeListener<Number>() {
							...
							public void changed(
									...
									Number oldValue,
									Number newValue) {
								double value = moveSpeedSlider
										.getValue();
								EvalConstants.THINKING_TIME = EvalConstants.MAX_THINKING_TIME
										- (value / 100
												* (EvalConstants.MAX_THINKING_TIME
														- EvalConstants.MIN_THINKING_TIME));
							}
						});
	...
	private double paintChessBoard(
			BitBoard board) {
		GraphicsContext g = chessPane
				.getGraphicsContext2D();
		double width = chessPane.getWidth();
		double height = chessPane.getHeight();
		double cellSize = Math.ceil(Math
				.min(width / 8.0, height / 8.0));
		int squareNo = 0;
				...
				if (piece != CoreConstants.EMPTY) {
					Image image = new Image(
							MainController.class
									.getResource(
											"/images/"
													+ CoreConstants.FILE_NAMES[piece]
													+ ".png")
									.toExternalForm());
					g.drawImage(image,
							col * cellSize,
	...
	public void setupGame() {
		...
		chessPane.setOnMouseClicked(
				evt -> clickListenerChessPane(
						board, evt, cellSize));
	}
	...
	private void clearCanvas() {
		GraphicsContext g = chessPane
				.getGraphicsContext2D();
		g.clearRect(0, 0, chessPane.getWidth(),
				chessPane.getHeight());
	}

	private void clickListenerChessPane(
			BitBoard board, MouseEvent evt,
			double cellSize) {
		...
		if (index < 64 && x <= (8 * cellSize)
			...
			if (piece != CoreConstants.EMPTY
				...
				ArrayList<Move> moves = getMovesPiece(
						index, moveList);
				oldPos = index;
				...
				blueSquares.clear();
				for (Move move : moves) {
					int rowMove = Math.floorDiv(
							move.getFinalPos(),
							8);
					int colMove = move
							.getFinalPos() % 8;
					if (board.board[move
							.getFinalPos()] == CoreConstants.EMPTY) {
	...
	public Move getMove(ArrayList<Move> moves,
			int piece, int oldIndex,
			int finalIndex) {
		for (Move move : moves) {
			if (move.getPieceType() == piece
					&& move.getOldPos() == oldIndex
	...
	private ArrayList<Move> getMovesPiece(
			int oldPos,
	...
	public void move(BitBoard board, Move move,
			boolean repaint) {
		boolean capture = board.board[move
				.getFinalPos()] != CoreConstants.EMPTY;
		board.move(move);
		...
		if (move.isPromotion()
				&& side == playerColour) {
			pawnPromotion(move.getOldPos(),
					move.getFinalPos(), side,
					board, true);
		...
		boolean playerLost = board
				.checkmate(playerColour);
		boolean stalemate = board
				.stalemate(board.toMove);
		...
		if (aiLost || playerLost || stalemate) {
			String result = "";
			String sPlayerColour = (playerColour == 0)
					? "WHITE" : "BLACK";
			if (aiLost) {
				result = "Congratulations you, playing with "
						+ sPlayerColour
						+ ", have won!";
			} else if (playerLost) {
	...
	private ArrayList<Move> getMoves(
			BitBoard board, boolean removeCheck) {
		return moveGen.generateMoves(board,
				removeCheck);
	}

	// Displays a dialog giving the player the choice of which piece to convert
	// their pawn to
	private void pawnPromotion(int pawnOldPos,
			int newPos, int side, BitBoard board,
			boolean display) {
		...
		if (display) {
			...
			dialog.setHeaderText(
					"Choose the piece to switch your pawn to");
			...
			if (result.isPresent()) {
				...
				pawnPromotion(pawnOldPos, newPos,
						side, board, display);
			}
			switch (choice) {
	...
	private void moveAI(BitBoard board) {
		...
		int colorFactor = (aiColor == 0)
				? EvalConstants.WHITE
				: EvalConstants.BLACK;
		Move move = search.rootNegamax(moveGen,
				board, colorFactor);
	...
	// chess game
	private void updatePGNTextField(
			BitBoard board, Move move,
			boolean capture) {
		String result = "";
		...
		if (side == 0) {
			result += String.valueOf(
					(board.getMoveNumber() / 2)
							+ 1)
					+ ". ";
		}
		// Add the letter of the piece being moved
		result += CoreConstants.pieceToLetterCapital[move
				.getPieceType()];
		...
		result += CoreConstants.indexToAlgebraic[move
				.getFinalPos()];
		...
		if (board.getMoveNumber() % 2 == 0
				&& board.getMoveNumber() != 0) {
			result += "\n";
		}
		pgnTextField.setText(
				(pgnTextField.getText() == null
						? ""
						: pgnTextField.getText())
						+ result);
		// Store history of the field so that undos work
		pgnHistory[board
				.getMoveNumber()] = pgnTextField
						.getText();
	}
		...
		if (file != null) {
			try {
				reader = new BufferedReader(
						new FileReader(file));
				board.toMove = Integer.valueOf(
						reader.readLine());
				int noOfMoves = Integer.valueOf(
						reader.readLine());
				board.setMoveNumber(noOfMoves);
				for (int i = 0; i <= 63; i++) {
					board.board[i] = (byte) ((int) Integer
							.valueOf(reader
									.readLine()));
				}
				for (int i = 0; i <= 13; i++) {
					board.bitboards[i] = Long
							.valueOf(reader
									.readLine());
				}
				...
				for (int i = 0; i < noOfMoves
						- 1; i++) {
					board.moveHistory[i] = Long
							.valueOf(reader
									.readLine());
					board.whiteHistory[i] = Long
							.valueOf(reader
									.readLine());
					board.blackHistory[i] = Long
							.valueOf(reader
									.readLine());
					board.pawnHistory[0][i] = Long
							.valueOf(reader
									.readLine());
					board.pawnHistory[1][i] = Long
							.valueOf(reader
									.readLine());
					board.rookHistory[0][i] = Long
							.valueOf(reader
									.readLine());
					board.rookHistory[1][i] = Long
							.valueOf(reader
									.readLine());
					board.queenHistory[0][i] = Long
							.valueOf(reader
									.readLine());
					board.queenHistory[1][i] = Long
							.valueOf(reader
									.readLine());
					board.bishopHistory[0][i] = Long
							.valueOf(reader
									.readLine());
					board.bishopHistory[1][i] = Long
							.valueOf(reader
									.readLine());
					board.knightHistory[0][i] = Long
							.valueOf(reader
									.readLine());
					board.knightHistory[1][i] = Long
							.valueOf(reader
									.readLine());
					board.kingHistory[0][i] = Long
							.valueOf(reader
									.readLine());
					board.kingHistory[1][i] = Long
							.valueOf(reader
									.readLine());
					board.boardHistory[0][i] = Byte
							.valueOf(reader
									.readLine());
					board.boardHistory[1][i] = Byte
							.valueOf(reader
									.readLine());
					board.castlingHistory[0][i] = Long
							.valueOf(reader
									.readLine());
					board.castlingHistory[1][i] = Long
							.valueOf(reader
									.readLine());
					board.epHistory[0][i] = Long
							.valueOf(reader
									.readLine());
					board.epHistory[1][i] = Long
							.valueOf(reader
									.readLine());
				}
				board.castling[0] = Integer
						.valueOf(reader
								.readLine());
				board.castling[1] = Integer
						.valueOf(reader
								.readLine());
				pgnTextField.setText(
						reader.readLine());
				playerColour = Integer.valueOf(
						reader.readLine());
				aiColor = Integer.valueOf(
						reader.readLine());
				clearCanvas();
				paintChessBoard(board);
				moveList = moveGen.generateMoves(
						board, true);
			} catch (Exception e) {
				Alert alert = new Alert(
						AlertType.ERROR);
	...
	private void handleSaveFileAction(
		...
		for (int i = 0; i < noOfMoves; i++) {
			result += String.valueOf(
					board.moveHistory[i]) + "\n";
			result += String.valueOf(
					board.whiteHistory[i]) + "\n";
			result += String.valueOf(
					board.blackHistory[i]) + "\n";
			result += String.valueOf(
					board.pawnHistory[0][i])
					+ "\n";
			result += String.valueOf(
					board.pawnHistory[1][i])
					+ "\n";
			result += String.valueOf(
					board.rookHistory[0][i])
					+ "\n";
			result += String.valueOf(
					board.rookHistory[1][i])
					+ "\n";
			result += String.valueOf(
					board.queenHistory[0][i])
					+ "\n";
			result += String.valueOf(
					board.queenHistory[1][i])
					+ "\n";
			result += String.valueOf(
					board.bishopHistory[0][i])
					+ "\n";
			result += String.valueOf(
					board.bishopHistory[1][i])
					+ "\n";
			result += String.valueOf(
					board.knightHistory[0][i])
					+ "\n";
			result += String.valueOf(
					board.knightHistory[1][i])
					+ "\n";
			result += String.valueOf(
					board.kingHistory[0][i])
					+ "\n";
			result += String.valueOf(
					board.kingHistory[1][i])
					+ "\n";
			result += String.valueOf(
					board.boardHistory[0][i])
					+ "\n";
			result += String.valueOf(
					board.boardHistory[1][i])
					+ "\n";
			result += String.valueOf(
					board.castlingHistory[0][i])
					+ "\n";
			result += String.valueOf(
					board.castlingHistory[1][i])
					+ "\n";
			result += String.valueOf(
					board.epHistory[0][i]) + "\n";
			result += String.valueOf(
	...
	private void undoAction(ActionEvent event) {
		if (board.getMoveNumber() >= 2) {
			board.undo();
			board.undo();
			pgnTextField.setText(pgnHistory[board
					.getMoveNumber()]);
			clearCanvas();
			paintChessBoard(board);
			moveList = moveGen
	...
	// This makes it easy
	@FXML
	private void loadFenMenuItem(
			ActionEvent event) {
		TextInputDialog dialog = new TextInputDialog(
				"");
		dialog.setTitle("Input FEN Notation");
		dialog.setContentText(
				"Please enter FEN of board to be loaded");
		...
		if (result.isPresent()) {
			try {
				...
				alert.setTitle("Error Dialog");
				alert.setContentText(
						"Ooops, there was an error whilst loading the FEN notation. "
	...
	@FXML
	private void exportFenMenuItem(
			ActionEvent event) {
		...
		alert.setHeaderText(
				"The FEN notation for the current board");
	...
	@FXML
	private void boardColourMenuItem(
			ActionEvent event) {
